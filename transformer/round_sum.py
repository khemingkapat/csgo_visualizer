...
def get_round_sum(dfs: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    if "kills" in dfs and not dfs["kills"].empty:
        ...
        for idx in kill_stats.index:
            if idx in rounds_sum_df.index:
                ...
                # )
                rounds_sum_df.loc[idx, "ct_hs_prob"] = kill_stats.loc[idx, "headshot"][
                    "ct_hs_prob"
                ]
                rounds_sum_df.loc[idx, "t_hs_prob"] = kill_stats.loc[idx, "headshot"][
                    "t_hs_prob"
                ]
                # rounds_sum_df.loc[idx, "ct_trade_prob"] = kill_stats.loc[
                #     idx, "revenge"
                # ]["ct_trade_prob"]
                # rounds_sum_df.loc[idx, "t_trade_prob"] = kill_stats.loc[idx, "revenge"][
                #     "t_trade_prob"
                # ]
                ...
                ct_kas = max(
                    kill_stats.loc[idx, "attacker_steamid"]["ct_unique_killers"],
                    kill_stats.loc[idx, "assister_steamid"]["ct_assisters"],
                    ct_survivors,
                    0,
                )
                t_kas = max(
                    kill_stats.loc[idx, "attacker_steamid"]["t_unique_killers"],
                    kill_stats.loc[idx, "assister_steamid"]["t_assisters"],
                    t_survivors,
                    0,
                )
                ...
                #     float
                # )
                rounds_sum_df.loc[idx, "ct_kas_prob"] = ct_kas / 5
                # rounds_sum_df["t_kast_prob"] = rounds_sum_df["t_kast_prob"].astype(
                #     float
                # )
                rounds_sum_df.loc[idx, "t_kast_prob"] = t_kas / 5

    # Process damages
    if "damages" in dfs and not dfs["damages"].empty:
    ...
    if "shots" in dfs and not dfs["shots"].empty:
        weapon_fires_df = dfs["shots"].reset_index()
    ...
    if "grenades" in dfs and not dfs["grenades"].empty:
        ...
        grenade_types_pivot = (
            grenades_df.groupby(["round_num", "thrower_side", "grenade_type"])
            .size()
            .unstack(["thrower_side", "grenade_type"], fill_value=0)
        )
    ...
    # Final processing
    rounds_sum_df["bomb_plant"] = rounds_sum_df["bomb_plant"].fillna(-1).astype(int)
    ...
    rounds_sum_df = pd.concat(
        [
            rounds_sum_df,
            pd.get_dummies(rounds_sum_df["reason"], prefix="reason").astype(int),
        ],
        axis=1,
    )
    ...
    rounds_sum_df["winning_side"] = (
        rounds_sum_df["winner"].str.lower() == "ct"
    ).astype(int)
